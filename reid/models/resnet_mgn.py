...
__all__ = [ 'ResNet50_mgn', 'ResNet101_mgn']
...
class ResNet_mgn(nn.Module):
    ...
    def __init__(self, depth, num_features=0, norm=False,
                 dropout=0, num_classes=751, add_l3_softmax=False, test_stage=False):

        super(ResNet_mgn, self).__init__()
        block = Bottleneck
        if depth not in ResNet_mgn.__factory:
            raise KeyError("Unsupported depth:", depth)
        layers = ResNet_mgn.__factory[depth]
        self.inplanes = 64
...
########################################test###############################################################
    ...
    def forward(self, x):
        ...
        # # g1_mp_v = g1_mp.view(g1_mp.size(0),-1)
        ...
        # scores = [g1_sm]
        # fea_trip = [g1_rd]
        # fea_for_test = g1_mp_s
        # scores = [g1_sm, g2_sm,g3_sm]
        # fea_trip = [g1_rd,g2_rd,g3_rd]
        # fea_for_test = torch.cat([g1_mp_s,g2_mp_s,g3_mp_s],1)
        # scores = [p2_1_sm, p2_2_sm]
        # fea_trip = [g2_rd]
        # fea_for_test = torch.cat([p2_1_rd,p2_2_rd],1)
        ...
        g1_mp = F.avg_pool2d(g1, g1.size()[2:])
        g1_rd = self.g1_reduce(g1_mp)  # 2048->256
        g1_rd = g1_rd.squeeze() # it is different from paper
...
def ResNet50_mgn(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_mgn( 50, **kwargs)
    if pretrained:
        # pdb.set_trace()
        copy_weight_for_branches('resnet50',model)
        model.load_state_dict(torch.load('./resnet50_mgn.tar'))
    return model


def ResNet101_mgn(pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_mgn(100, **kwargs)
    if pretrained:
        copy_weight_for_branches('resnet101',model)
        model.load_state_dict(torch.load('./resnet101_mgn.tar'))
    return model

def ResNet_mgn_model(depth, pretrained=True, **kwargs):
    if depth==50:
        print('resnet50-mgn')
        return ResNet50_mgn(pretrained=pretrained, **kwargs)
    elif depth==101:
        print('resnet101-mgn')
        return ResNet101_mgn(pretrained=pretrained, **kwargs)
    else:
        raise KeyError("Unsupported depth:", depth)
