...
import torchvision
import pdb
from .layer import convLR, convDU, MultiHeadAttention
...
class ResNet_channel(nn.Module):
    __factory = {
    ...
    def __init__(self, depth, pretrained=True, num_features=0, 
        ...
        super(ResNet_channel, self).__init__()
        ...
        if depth not in ResNet_channel.__factory:
            raise KeyError("Unsupported depth:", depth)
        self.base = ResNet_channel.__factory[depth](pretrained=pretrained)
        ...
        init.kaiming_normal_(self.feat_l3.weight, mode='fan_out')
        init.constant_(self.feat_l3.bias, 0)
        init.constant_(self.feat_bn_l3.weight, 1)
        init.constant_(self.feat_bn_l3.bias, 0)
        ...
        init.normal_(self.classifier_l4.weight, std=0.001)
        init.constant_(self.classifier_l4.bias, 0)
        init.normal_(self.classifier_l3.weight, std=0.001)
        init.constant_(self.classifier_l3.bias, 0)
    ...
    def forward(self, x):
        ...
        channel_prior = ll_attention_like_map.view(ll_attention_like_map.size(0),ll_attention_like_map.size(1),-1).transpose(2,1)

        x_channel, attention_weight = self.channel_attention(channel_prior,channel_prior,channel_prior)

        x_channel = x_channel.transpose(2,1).view(x.size())


        x = F.avg_pool2d(x_channel, x_channel.size()[2:])
        x = x.view(x.size(0), -1)
        ...
        if self.add_l3_softmax:
            ...
            if self.test_stage:
                return attention_weight,cls_out,trip_out,l2_side,l3_side,l4_side,x
