...
__all__ = [ 'resnet50_stn', 'resnet101_stn']
...
class ResNet_stn(nn.Module):
    ...
    def __init__(self, block, layers, num_features=0, norm=False, dropout=0, num_classes=751):
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,is_last=True)
        # self.layer3_0 = self._make_layer(block, 256, layers[2], stride=2,is_last=False)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,is_last=True)
        # self.layer4_0 = self._make_layer(block, 512, layers[3], stride=2,is_last=False)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1,is_last=True)

        self.delf_cbr1 = self._cbr(out_channels=512)
        self.delf_conv2 = nn.Conv2d(512,1,kernel_size=1,stride=1,padding=0)

        self.classifier_test = nn.Linear(2048, num_classes)
    ...
    def forward(self, x):
        ...
        g1_delf1 = self.delf_cbr1(g1_l)
        g1_delf_att = self.delf_conv2(g1_delf1)
        g1_1_att = g1_l * g1_delf_att

        # g1_1_att = g1_l

        g1_mp = F.avg_pool2d(g1_1_att, g1_1_att.size()[2:])
        g1_rd = g1_mp.squeeze()
        # g1_rd = self.g1_reduce(g1_mp)  # 2048->256
        # g1_rd = g1_rd.squeeze()
        # g1_sm = self.classifier_g1(g1_rd)
        g1_sm = self.classifier_test(g1_rd)
...
def copy_weight_for_branches(model_name, new_model, branch_num=2):
    ...
    for block in new_model_keys:
        ...
        if(ori_prefix == 'layer2') or (ori_prefix == 'layer3') or (ori_prefix =='layer4'):
            ...
            new_model_weight[block] = model_weight[ori_key]
    save_name = model_name + '_stn.tar'
    torch.save(new_model_weight,save_name)


def resnet50_stn(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_stn(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        # pdb.set_trace()
        copy_weight_for_branches('resnet50',model,branch_num=1)
        model.load_state_dict(torch.load('./resnet50_stn.tar'))
    return model


def resnet101_stn(pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_stn(Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        copy_weight_for_branches('resnet101',model,branch_num=1)
        model.load_state_dict(torch.load('./resnet101_stn.tar'))
    return model
