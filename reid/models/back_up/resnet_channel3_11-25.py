...
__all__ = ['ResNet_channel3']

#############################l4 multipath, and remove stride =2 in layer4 reomve l42_sideloss by l2_side = l4_side and multiply 
#!!!! in resnet_channel3 l2_side is meaningless!!!!
#!!!! in resnet_channel3 l2_side is meaningless!!!!
#!!!! in resnet_channel3 l2_side is meaningless!!!!
#!!!! in resnet_channel3 l2_side is meaningless!!!!
#!!!! in resnet_channel3 l2_side is meaningless!!!!
class ResNet_channel3(nn.Module):
    __factory = {
    ...
    def __init__(self, depth, pretrained=True, num_features=0, 
        ...
        super(ResNet_channel3, self).__init__()
        ...
        if depth not in ResNet_channel3.__factory:
            raise KeyError("Unsupported depth:", depth)
        base = ResNet_channel3.__factory[depth](pretrained=pretrained)
        ...
        self.layer4 = make_layer(inplanes=1024, planes=512, blocks=3, stride=1, groups = self.num_bypath)
        # pdb.set_trace()
        
        for submodule_new, submodule_old in zip(self.layer4.children(),base.layer4.children()) :
            parallel_bypath( submodule_new, submodule_old, self.num_bypath)


        self.layer2_ca = MultiHeadAttention(512, 4)
        self.layer3_ca = MultiHeadAttention(1024, 4)
        self.layer4_ca = MultiHeadAttention(2048, 4)
    ...
    def forward(self, x):
        ...
        # l2_ca = self.layer2_br(l2_ca)
        # l2_relu = F.relu(l2_ca,inplace=True)
        ...
        # l3_ca = self.layer3_br(l3_ca)
        # l3_relu = F.relu(l3_ca,inplace=True)
        ...
        # l3_side = F.normalize(l3_ca.pow(2).mean(1).view(l3_ca.size(0),-1)).view(l3_ca.size(0),1,l3_ca.size(2),l3_ca.size(3))  #batch*1*8*4
        ...
        # l4_side = F.normalize(l4_fsa.pow(2).mean(1).view(l4_fsa.size(0),-1)).view(l4_fsa.size(0),1,l4_fsa.size(2),l4_fsa.size(3))  #batch*1*8*4
        # SideAdd = torch.add(l3_side, l4_side)
        # l4_fsa = torch.mul(l4_fsa, SideAdd)
        l4_fea.append(l4_fsa)

        # feature for channel attention
        l4_fca = l4_outs[1]
        l4_fsa_reshape = l4_fsa.view(l4_fca.size(0),l4_fca.size(1),-1).transpose(2,1)
        ...
        # l4_ca = self.layer4_br(l4_ca)
       # l4_relu = F.relu(l4_ca,inplace=True)
        ...
        # 2018-11-18 add softmax feature to final features to consider the test set and train set distribution?
        g_rd = [l2_rd,  l3_rd, ] + l4_rd
        g_sm = [l2_sm, l3_sm] + l4_sm
        ...
        #!!!! in resnet_channel3 l2_side is meaningless!!!!
        if self.test_stage:
            return [l2_attw,l3_attw,l4_attw], g_sm, g_rd, l4_side, l3_side, l4_side, net_rd
