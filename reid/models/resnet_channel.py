...
class ResNet_channel(nn.Module):
    ...
    def forward(self, x):
        ...
        channel_prior = l4_maxp.view(l4_maxp.size(0),l4_maxp.size(1),-1).transpose(2,1)
        ...
        l4_side = F.normalize(x_channel.pow(2).mean(1).view(x_channel.size(0),-1)).view(x_channel.size(0),1,x_channel.size(2),x_channel.size(3))  #batch*1*8*4
        
        ll_concate1 = torch.add(l2_side,l3_side)
        ll_concate = torch.add(ll_concate1,l4_side)
       
        ll_attention_like_map = torch.mul(x_channel,ll_concate)
