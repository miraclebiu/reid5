...
loss_config.pnsm_weight = False
#triplet weight combineori triplet list and weight
loss_config.pnsm_com = False
...
loss_config.pnsm_margin = 2.0
# shrink the radius of the positive set 
loss_config.pnsm_pos_radius_divide = 2.0
# shrink the radius of the negative set
loss_config.pnsm_neg_radius_divide = 2.0
...
def select_logsumexp_triplet(feature, targets, num_instances):
    n = feature.size(0)
    num_person = n // num_instances
    # Compute pairwise distance, replace by the official when merged
    dist = torch.pow(feature, 2).sum(1).expand(n, n)
    ...
    for i in range(n):
        positives = dist[i][mask[i]]
        # positives_delself = remove_self(positives, i%num_instances, num_instances)
        LogSumExp_pos = positives.exp().sum().log()
        positive_hard = LogSumExp_pos
        pos_hard = positives.max()


        negatives = dist[i][mask[i] == 0]
        LogSumExp_neg = (-negatives).exp().sum().log() # calculate minimal value
        negative_hard = -LogSumExp_neg
        neg_hard = negatives.min()
        ### max(x,0) similar log(1+x)

        dist_ap_hard.append(positive_hard)
        dist_an_hard.append(negative_hard)

        dist_mean_most.append(pos_hard - neg_hard)
        dist_mean_appro.append(positive_hard-negative_hard)

    dist_ap_hard = torch.stack(dist_ap_hard)
    dist_an_hard = torch.stack(dist_an_hard)

    dist_mean_most = torch.stack(dist_mean_most)
    dist_mean_appro = torch.stack(dist_mean_appro)
    print('mean, most:  {:.2f} appro: {:.2f}'.format(dist_mean_most.mean().item(),dist_mean_appro.mean().item()) )
...
## positive negative set margin loss
def select_pnsm_normalize(feature, targets, num_instances,
                            normalizer,
                            pos_margin, pos_radius_divide,
                            neg_margin, neg_radius_divide,pnsm_com=False):
    n = feature.size(0)
    fea_dim = feature.size(1)
    scale = fea_dim ** -0.5
    ...
    for i in range(n):
        ...
        dist_mean_pn.append(positive_weighted-negative_weighted)
        dist_mean_trip.append(positive_hard - negative_hard)
    ...
    dist_mean_pn = torch.stack(dist_mean_pn)
    dist_mean_pn = dist_mean_pn.mean()
    dist_mean_trip = torch.stack(dist_mean_trip)
    dist_mean_trip = dist_mean_trip.mean()
    print("triplet_dist: {:.2f}, pnSet_dist: {:.2f}".format(dist_mean_trip.item(), dist_mean_pn.item()))
    # print("pos_neg_triplet_mean_dist: {:.2f}".format(dist_mean2.item()))
...
def select_quadruplet_normalize(feature, targets, num_instances):
    ...
    for i in range(n):
        ...
        dist_mean1.append(hard_negative[0]-hard_positive)
        # print(mask[i])
        # print(mask2)
        # pdb.set_trace()
    ...
    # dist_mean1 = torch.stack(dist_mean1)
    # dist_mean1 = dist_mean1.mean()
    # print("pos_neg_hardSet_mean_dist: {:.2f}".format(dist_mean1.item()))
...
def cal_rank_loss_weight_normalize(feature, targets, num_instances, margin):
    n = feature.size(0)
    fea_dim = feature.size(1)
    scale = fea_dim ** -0.5
...
class Loss_Single(nn.Module):
    def __init__(self, config):
        ...
        self.pos_divide = config.pnsm_pos_radius_divide
        self.neg_divide = config.pnsm_neg_radius_divide
        if config.pnsm_weight and not config.trip and not config.quad :
            self.ranking_loss2.margin = config.pnsm_margin
    ...
    def forward(self, inputs, targets):
        ...
        ############ triplet
        if self.config.trip and not self.config.pnsm_weight and not self.config.quad and not self.config.rank_loss:
        ...
        #### triplet weight
        if  self.config.pnsm_weight and not self.config.trip and not self.config.quad and not self.config.rank_loss:
            normalizer = nn.Softmax(dim=0)
            dist_ap, dist_an, y = select_pnsm_normalize(trip_out,targets,
                                                           self.num_instances,
                                                           normalizer,
                                                           self.config.pnsm_margin, self.config.trip_weight_pos_radius_divide,
                                                           self.config.pnsm_margin, self.config.trip_weight_neg_radius_divide,self.config.pnsm_com)
            if self.config.pnsm_com:
        ...
        ####### quad
        if self.config.quad and not self.config.pnsm_weight and not self.config.trip and not self.config.rank_loss:
            #### print('Using quadruplet loss')
            dist_ap, dist_an, dist_ann, y = select_quadruplet_normalize(trip_out, targets, self.num_instances)
            TripletLoss1 = self.ranking_loss1(dist_an, dist_ap, y)
            TripletLoss2 = self.ranking_loss2(dist_ann, dist_ap, y) 
            prec = prec = (dist_an.data > dist_ap.data).sum().item() * 1. / y.size(0)
        ##### rank
        if self.config.rank_loss and not self.config.pnsm_weight and not self.config.trip and not self.config.quad:
            rank_loss, prec = cal_rank_loss_weight_normalize(trip_out, targets, self.num_instances, self.config.margin1)
...
class Loss_Multi(nn.Module):
    def __init__(self, config):
        ...
        self.neg_divide = config.pnsm_neg_radius_divide
        if config.pnsm_weight and not config.trip and not config.quad :
            self.margin2 = config.pnsm_margin
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(0,num_trip):
            if self.config.trip and not self.config.pnsm_weight and not self.config.quad and not self.config.rank_loss:
                ...
                dist_ap, dist_an, y = select_logsumexp_triplet(trip_out[i],targets,
                                                     self.num_instances)
            ...
            if  self.config.pnsm_weight and not self.config.trip and not self.config.quad and not self.config.rank_loss:
                normalizer = nn.Softmax(dim=0)
                dist_ap, dist_an, y = select_pnsm_normalize(trip_out[i],targets,
                                                               self.num_instances,
                                                               normalizer,
                                                               self.config.pnsm_margin, self.config.pnsm_pos_radius_divide,
                                                               self.config.pnsm_margin, self.config.pnsm_neg_radius_divide,self.config.pnsm_com)
            ...
            if self.config.quad and not self.config.pnsm_weight and not self.config.trip and not self.config.rank_loss:
                dist_ap, dist_an, dist_ann, y = select_quadruplet_normalize(trip_out[i], targets, self.num_instances)
                TripletLoss1 += F.margin_ranking_loss(dist_an, dist_ap, y, self.margin1)
            ...
            if self.config.rank_loss and not self.config.pnsm_weight and not self.config.trip and not self.config.quad:
                rank_loss, prec = cal_rank_loss_weight_normalize(trip_out[i], targets, self.num_instances, self.config.margin1)
                TripletLoss1 = rank_loss
