...
def re_ranking(probFea,galFea,k1=20,k2=6,lambda_value=0.3, MemorySave = False, Minibatch = 2000):
    ...
    print('save_ori_dist')
    np.save('original_dist.npy',original_dist)
    V = np.zeros_like(original_dist).astype(np.float16)
    initial_rank = np.argsort(original_dist).astype(np.int32)
    print('save_ori_rank')
    np.save('initial_rank.npy',initial_rank)
    # gallery_num = original_dist.shape[0]

    # original_dist = np.load('original_dist.npy')
    # V = np.zeros_like(original_dist).astype(np.float16)
    # initial_rank = np.load('initial_rank.npy')
    # gallery_num = original_dist.shape[0]
